...
除根因定位精度外，诊断效率也是衡量方法实用性的重要维度。本文方法引入了多智能体多轮协作、多模态特征计算以及基于 LLM 的推理（含逃逸机制），会带来相应的推理延迟与 Token 消耗。表 \ref{tab:diagnosis_cost} 在三个数据集的测试集上统计了端到端诊断开销。

\begin{table}[htbp]
    \centering
    \bicaption{\enspace 端到端诊断开销统计}{\enspace End-to-End Diagnosis Cost Statistics}
    \label{tab:diagnosis_cost}
    \ChapFiveTableSetup
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lccccc}
        \toprule
        \rowcolor{\ChapFiveTableHeadColor}
        \textbf{数据集} & \textbf{指纹构建 (s)} & \textbf{SOP 召回 (s)} & \textbf{工具调用次数} & \textbf{Tokens (K)} & \textbf{总耗时 (s)} \\
        \midrule
        AIOps-25 & 12.8 & 0.87 & 116 & 34.2 & 291 \\
        Bank & 11.4 & 0.73 & 102 & 32.5 & 267 \\
        Market & 14.6 & 0.94 & 138 & 35.8 & 318 \\
        \bottomrule
    \end{tabular}%
    }
\end{table}

从表 \ref{tab:diagnosis_cost} 可以看出，三个数据集上的总诊断耗时分别为 291 s、267 s 和 318 s，约 4.5--5.3 分钟。故障指纹构建约需十余秒，SOP 召回不到 1 s，二者合计占比较小。工具调用次数与系统拓扑规模呈正相关——Market 数据集平均包含 44.0 个服务，其工具调用次数（138 次）也最高。单次诊断的 Token 消耗约为 34K 左右。

图 \ref{fig:diagnosis_cost_breakdown} 给出了各阶段耗时占比。从耗时分解看，LLM 推理和工具调用是主要开销来源，二者合计占总耗时的 80\% 以上。故障指纹构建和 SOP 召回虽然耗时十余秒，但在总耗时中占比不到 5\%。在成本控制方面，系统采取了以下措施：故障指纹检索和 SOP 召回的结果可跨同类告警共享缓存；诊断过程中的观测查询支持按感知域并行发起；每次在线诊断设置最大轮数上限和 Token 预算，超出时终止并返回当前最优结论。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{Figures-Src/Chap5-Experiment/diagnosis-cost/chart.pdf}
    \bicaption{\enspace 诊断耗时阶段分解}{\enspace Diagnosis Time Breakdown by Stage}
    \label{fig:diagnosis_cost_breakdown}
\end{figure}
...
端到端根因定位实验显示，本文方法在三个数据集上的 Hit@1、Hit@3、Hit@5 和 MRR 均为最佳。与最强的大语言模型基线 Flow-of-Action 相比，本文方法的 Hit@1 仍保持了 10.50 到 12.30 个百分点的稳定优势。诊断开销分析表明，端到端诊断耗时约 4.5--5.3 分钟，单次 Token 消耗约 34K，主要开销来自 LLM 推理与工具调用。消融实验进一步说明，SOP 引导、逃逸机制和焦点与感知域约束都会影响最终性能，其中 SOP 引导贡献最大；移除该机制后，系统性能会明显回落到通用大语言模型智能体附近。
